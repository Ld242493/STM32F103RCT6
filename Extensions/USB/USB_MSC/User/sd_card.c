 ...
 * @return uint8_t 
 */
uint8_t SPI_ReadWrite(uint8_t txbyte)
{
  uint8_t rxbyte;
...
{
  __HAL_SPI_DISABLE(&hspi1);                    //修改前需要先关闭SPI
  hspi1.Instance->CR1 &= 0xFFC7;                //清空  位3~5
  hspi1.Instance->CR1 |= BaudRatePrescaler;     //设置波特率
  __HAL_SPI_ENABLE(&hspi1);                     //修改后打开SPI
}
